...
official-template layer, and the fail-closed submission gate established by
thesis ticket 01. The current PDF is a minimal template/build shell, not a
submission candidate and not continuous chapter prose.
...
and heading hierarchy. A successful draft build does not imply submission
readiness.
...
entry, or a bibliography entry is never cited. Failure is expected for the
ticket-01 shell.
