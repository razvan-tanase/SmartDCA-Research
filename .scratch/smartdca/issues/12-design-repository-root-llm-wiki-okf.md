...
| `sources` | Canonical/evidence concepts unless `original_record: true` | List of mappings. Every entry requires `id`, `title`, and OKF-required `resource`. Footnote labels join to `id`. Optional source extensions are `retrieved_at`, `upstream_version`, and `sha256`. |
...
Imported external sources are immutable after ingestion. Exact upstream Markdown bytes are stored under a non-`.md` suffix such as `references/raw/<source>/<version>/source.md.raw` or in an archive, while a separate conformant source-summary concept links to them. The SHA-256 fingerprints the raw upstream bytes, not the wrapper concept. When redistribution is inappropriate, the wiki retains authoritative URL provenance without an unauthorized local copy. Canonical high-risk claims use Markdown footnotes joined to `sources[].id`; internal concepts remain governed, versioned, editable knowledge.
