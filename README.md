...
Thesis manuscript tickets 01--13 are now resolved: the institutional contract, canonical build, thesis architecture, and evidence controls are frozen; Chapter 2 has reviewed investment, mean-theory, and computational-method synthesis; Chapter 3 establishes the fair same-deposit model and corrected-signal foundations; Chapter 4 with Appendix A carries the reviewed causal impossibility theorem, sharp epsilon-DCA guardrail, and complete guarded policy; Chapter 5 with Appendix B gives the reviewed finite witnesses, arbitrary-horizon accounting, qualified cash-crossing mechanism, and exact realized-ledger boundary; Chapter 6 with Appendices C--D defines the frozen empirical design, inference limits, provenance model, versioned artifact lifecycle, and reproduction routes; Chapter 7 with Appendix E reports the reviewed deterministic, adversarial, and three-seed stochastic evidence through four reproducible assets without pooling evidential layers; and Chapter 8 with Appendix E now reports the reviewed primary historical and separately registered robustness evidence through four generated assets with independent domain, statistical-language, reproducibility, and visual review. Chapter 9 now synthesizes the proved floor, path-dependent adaptivity, negative historical finding, limitations, and separately scoped future questions on reconciled evidence and contribution/non-claim controls. The introduction, closing literature synthesis, conclusion, and English abstract now align the approved questions, contributions, non-claims, and historical headlines on reviewed evidence. The current source uses the requested single English cover and English abstract; the retained institutional contract still records the template's bilingual route pending supervisor/faculty confirmation. Ticket 14 now has a complete integrated draft, generated evidence map, and reproducible review-package command; it remains open because the frozen institutional inputs and front-matter placeholders still block its release-candidate acceptance criteria. See the [assembly audit](research/notes/complete-manuscript-assembly-audit.md). The [effort
...
control/build/release tests on every push and pull request. The twenty-eight
...
scientific suite. The complete integrated draft is intentionally not a submission
